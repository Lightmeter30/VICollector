...
def build_manifest(session: Path) -> dict[str, object]:
    ...
    return {
        ...
        "next_step": "Map VICollector CSV rows to EuRoC mav0/cam0 and mav0/imu0 layout.",
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Create a manifest for future EuRoC export.")
    parser.add_argument("session", type=Path)
    parser.add_argument("output", type=Path)
    args = parser.parse_args()
    manifest = build_manifest(args.session)
    args.output.mkdir(parents=True, exist_ok=True)
    (args.output / "export_manifest_euroc.json").write_text(
